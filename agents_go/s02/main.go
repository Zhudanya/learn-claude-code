// s02 - Tool Use (Go version)
// dispatch map + 专用工具（bash, read_file, write_file, edit_file）
// 运行: cd agents_go && go run ./s02/
...
func init() {
	...
	systemMsg = fmt.Sprintf("You are a coding agent at %s. Use tools to solve tasks. Act, don't explain.", workdir)
...
func runEdit(path, oldText, newText string) string {
	...
	os.WriteFile(fp, []byte(strings.Replace(content, oldText, newText, 1)), 0644)
	return fmt.Sprintf("Edited %s", path)
}

// dispatch map: 工具名 → 执行函数
...
func main() {
	...
	for {
		fmt.Print("\033[36ms02-go >> \033[0m")
		if !scanner.Scan() {
			break
		}
